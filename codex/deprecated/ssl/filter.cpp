...
#include <codex/ssl/filter.hpp>
#include <openssl/err.h>
#include <codex/ssl/error_category.hpp>
...
void filter::end_handshake( void ) {
	if ( _handshake_state == k_handshake_accept ) {
		codex::io::ip::tcp::filter::on_accepted( _accept_addr );
	}
	if ( _handshake_state == k_handshake_connect ) {
		codex::io::ip::tcp::filter::on_connected();
	}
	_handshake_state = 0;
}

void filter::on_accepted( const codex::io::ip::address& addr ){
...
void filter::send_pending( void ) {
	while ( BIO_ctrl_pending( _wbio )) {
		codex::buffer::memory_block mb( 4096 );
...
bool filter::read_data_write_bio( codex::buffer::memory_block& msg ) {
	while( msg.length() > 0 ) {
...
bool filter::write_data_write_ssl( codex::buffer::memory_block& msg ) {
	while( msg.length() > 0 ) {
...
void filter::on_read( codex::buffer::memory_block& msg ){
	...
	while ( true ) {
		...
		if ( len <= 0 ) {
			...
			mb.wr_ptr( len );
			codex::io::ip::tcp::filter::on_read( mb );
		}		
	}
	send_pending();
}

void filter::do_write( codex::buffer::memory_block& msg ){
