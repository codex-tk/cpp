#ifndef __codex_util_rtti_h__
#define __codex_util_rtti_h__
...
public:
	...
	class get_handler {
	public:
		typedef const codex::util::rtti* (*handler_function)();

		get_handler( void );
		~get_handler( void );

		const codex::util::rtti* operator()( void ) const ;
...
public: 															\
	codex::util::rtti::get_handler	 rtti;							\
	static const codex::util::rtti	 s_rtti;						\
	static const codex::util::rtti* _rtti_impl( void) {				\
		return &class_name::s_rtti;									\
	}																\
	template < typename type >										\
	bool is_kind_of() {												\
		const codex::util::rtti* r = rtti();							\
		while ( r != nullptr ){										\
			if ( r == &type::s_rtti ) {								\
				return true;										\
			}														\
...
public:																\
	static  const codex::util::rtti  s_rtti;							\
	virtual const codex::util::rtti* rtti( void ){					\
		return &class_name::s_rtti;									\
	}																\
	template < typename type >										\
	bool is_kind_of() {												\
...
#define IMPL_ROOT_RTTI( class_name )				const codex::util::rtti class_name::s_rtti( #class_name , nullptr );
#define IMPL_CHILD_RTTI( class_name , parent_name ) const codex::util::rtti class_name::s_rtti( #class_name , &parent_name::s_rtti );
...
TEST( rtti_test , no_virtual ) {
	...
	std::cout << codex::util::rtti::get<no_virtal::parent>( root )->name() << std::endl;
	delete root;
