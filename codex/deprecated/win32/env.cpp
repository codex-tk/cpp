#include "stdafx.h"
#include <codex/util/win32/env.hpp>
...
codex::tstring         __stdcall env::process_name( void ) {
	TCHAR filePath[MAX_PATH];
	GetModuleFileName( nullptr , filePath , MAX_PATH );
	TCHAR *fileName = _tcschr( filePath , '\\' );
	if ( fileName ) {
		return codex::tstring( fileName );
	}
	return codex::tstring( _T("") );
}

codex::tstring         __stdcall env::process_directory( void ){
	TCHAR filePath[MAX_PATH];
	GetModuleFileName( nullptr , filePath , MAX_PATH );
...
codex::tstring         __stdcall env::current_directory( void ){
	TCHAR filePath[MAX_PATH];
	GetCurrentDirectory( MAX_PATH , filePath );
	return codex::tstring( filePath );
}


codex::tstring __stdcall env::process_path( void ) {
	TCHAR path[MAX_PATH];
	GetModuleFileName( nullptr , path , MAX_PATH );
	return codex::tstring( path );
}

codex::tstring		__stdcall env::os_version_string( void ) {
	switch ( os_version() ) {
