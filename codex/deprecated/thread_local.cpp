#include "stdafx.h"
#include <codex/threading/thread_local.hpp>
#include <cassert>
...
thread_local_impl::thread_local_impl( void ) 
{
	...
	if ( _key == TLS_OUT_OF_INDEXES ) {
		CODEX_ASSERT( !"[codex::threading::thread_local_impl::thread_local_impl()] TlsAlloc Failed" );
	}
}
...
void thread_local_impl::set( thread_local_impl::value_type value ) {
	assert_msg( _key != TLS_OUT_OF_INDEXES 
		, "[codex::threading::thread_local_impl::set()] TlsAlloc Failed" );
	if ( _key != TLS_OUT_OF_INDEXES )
...
thread_local_impl::value_type thread_local_impl::get( void ) {
	assert_msg( _key != TLS_OUT_OF_INDEXES 
		, "[codex::threading::thread_local_impl::get()] TlsAlloc Failed" );
	if ( _key != TLS_OUT_OF_INDEXES )
...
thread_local_impl::thread_local_impl( void ) 
{
	...
	case EAGAIN:
		CODEX_ASSERT( !"[codex::threading::thread_local_impl::thread_local_impl()] pthread_key_create Failed" );
		break;
	}	
}
